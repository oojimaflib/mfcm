...
#ifndef mf_parser_read_hpp
#define mf_parser_read_hpp

#include <boost/property_tree/ptree.hpp>
#include "mf_parser_config.hpp"
#include "mf_parser_error.hpp"
...
namespace boost {
  namespace property_tree {
    namespace mf_parser {
        ...
        while (b != e)
	  {
            if (*b == Ch('\\'))
	      {
                ...
                if (b == e)
		  {
                    BOOST_PROPERTY_TREE_THROW(mf_parser_error("character expected after backslash", "", 0));
		  }
		  ...
		  BOOST_PROPERTY_TREE_THROW(mf_parser_error("unknown escape sequence", "", 0));
	      }
            else
	      result += *b;
            ++b;
      ...
      std::basic_string<Ch> read_word(const Ch *&text)
      {
        ...
        const Ch *start = text;
        while (!is_ascii_space(*text) && *text != mf_comment_char && *text != Ch('\0'))
	  ++text;
      ...
      std::basic_string<Ch> read_line(const Ch *&text)
      {
        ...
        skip_whitespace(text);
        const Ch *start = text;
        while (*text != Ch('\0') && *text != mf_comment_char)
	  ++text;
        ...
        if (*text == Ch('\"'))
	  {
		  ...
		  {
                    if (!need_more_lines)
		      BOOST_PROPERTY_TREE_THROW(mf_parser_error("unexpected \\", "", 0));
                    ++text;
                    skip_whitespace(text);
                    if (*text == Ch('\0') || *text == mf_comment_char)
		      *need_more_lines = true;
                    else
		      BOOST_PROPERTY_TREE_THROW(mf_parser_error("expected end of line after \\", "", 0));
		  }
                else
		  if (need_more_lines)
	      ...
	      BOOST_PROPERTY_TREE_THROW(mf_parser_error("unexpected end of line", "", 0));

	  }
        else
	  BOOST_PROPERTY_TREE_THROW(mf_parser_error("expected \"", "", 0));
      }
	  ...
	  std::basic_string<Ch> keyword = read_word(text);
	  while (keyword.size() > 0 && keyword != mf_keyval_delimiter<Ch>) {
	    key += keyword + mf_keyword_sep<Ch>;
	    keyword = read_word(text);
	  }
	  if (key.size() > 0) {
      ...
      void read_mf_internal(std::basic_istream<Ch> &stream,
			    Ptree &pt,
	...
	try {
	  ...
	  while (stream.good()) {
	    ...
	    if (!stream.good() && !stream.eof())
	      BOOST_PROPERTY_TREE_THROW(mf_parser_error("read error", filename, line_no));
		...
		if (include_depth > 100) {
		  BOOST_PROPERTY_TREE_THROW(mf_parser_error("include depth too large, "
							    "probably recursive include",
							    filename, line_no));
		}
		str_t s = read_string(text, NULL);
		std::string inc_name = convert_chtype<char, Ch>(s.c_str());
		stdfs::path inc_path = inc_name;

		// If we're including a relative path, it should be
		// relative to the location of this file, not the
		// current directory.
		if (inc_path.is_relative()) {
		  stdfs::path thisfile_path = filename;
		  stdfs::path root_path = stdfs::absolute(thisfile_path.parent_path());
		  inc_path = root_path / inc_path;
		}
		
		std::basic_ifstream<Ch> inc_stream(inc_path.c_str());
		if (!inc_stream.good())
		  BOOST_PROPERTY_TREE_THROW(mf_parser_error("cannot open include file " + inc_name,
							    filename, line_no));
		read_mf_internal(inc_stream, *stack.top(),
				 inc_name, include_depth + 1);
	      } else {   // Unknown directive
		BOOST_PROPERTY_TREE_THROW(mf_parser_error("unknown directive", filename, line_no));
	      }

	      // Directive must be followed by end of line
	      skip_whitespace(text);
	      if (*text != Ch('\0')) {
		BOOST_PROPERTY_TREE_THROW(mf_parser_error("expected end of line", filename, line_no));
	      }
	      ...
	      case s_key:
		{
		  ...
		  if (*text == Ch('{'))   // Brace opening found
		    ...
		    {
		      if (stack.size() <= 1)
			BOOST_PROPERTY_TREE_THROW(mf_parser_error("unmatched }", "", 0));
		      stack.pop();
		      last = NULL;
		      ++text;
		    }
	      ...
	      case s_data_cont:
		{
		  ...
		  if (*text == Ch('\"'))  // Continuation must start with "
		    ...
		    BOOST_PROPERTY_TREE_THROW(mf_parser_error("expected \" after \\ in previous line", "", 0));
	...
	catch (mf_parser_error &e)
	  {
	    // If line undefined rethrow error with correct filename and line
	    if (e.line() == 0)
	      {
		BOOST_PROPERTY_TREE_THROW(mf_parser_error(e.message(), filename, line_no));
	      }
	    else
