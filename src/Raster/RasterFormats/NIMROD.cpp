/***********************************************************************
 * mfcm Raster/RasterFormats/NIMROD.cpp
 *
...
/**
   Returns whether the host system is little-endian (true) or
   big-endian (false).
 */
...
/**
   Reverse the endianness of a datum.

   @tparam N The length of the datum to reverse in bytes.

   @param in Pointer to datum.
   @param out Pointer to location for reversed datum.
 */
template<uint8_t N>
...
/**
   Read a big-endian datum from an input stream on a little-endian
   machine.

   @tparam T Type of the datum to read.
   @tparam N Length in bytes of the type of the datum to read
...
/**
   Read a big-endian datum from an input stream on a big-endian
   machine.
...
/**
   Read a big-endian datum from an input stream

   @tparam T Type of the datum to read.
   @tparam N Length in bytes of the type of the datum to read
   (defaults to sizeof(T))

   @param system_is_le Boolean indicating the endianness of the host
   system.
   @param is Input stream reference.
 */
template<typename T,
...
/**
   Read a fixed-length array of big-endian data from an input stream
   on a little-endian machine.

   @tparam T Type of the data to read.
   @tparam NT Number of data in the array.
   @tparam N Length in bytes of the type of the data to read
   (defaults to sizeof(T))

   @param is Input stream reference.
   @param arr Array reference for the resulting data.
 */
template<typename T,
	 size_t NT,
	 uint8_t N = sizeof(T)>
...
/**
   Read a variable-length array of big-endian data from an input
   stream on a little-endian machine. The number of data to read is
   determined by the length of the output vector object.
   ...
   @param is Input stream reference.
   @param arr Vector reference for the resulting data.
 */
template<typename T,
	 uint8_t N = sizeof(T)>
...
/**
   Read a variable-length array of big-endian data from an input
   stream on a big-endian machine. The number of data to read is
   determined by the length of the output vector object.
...
/**
   Read a variable-length array of big-endian data from an input
   stream. The number of data to read is determined by the length of
...
template<typename T>
template<typename U>
void NIMRODRasterFormat<T>::
read_nimrod_vector(const bool& sil,
		   std::istream& in_file)
{
  nxpx_ = h1_[16];
  nypx_ = h1_[15];
